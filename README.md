...
[![Release](https://img.shields.io/github/v/release/rebaze/rio)](https://github.com/rebaze/rio/releases/latest)
...
Start in a project whose build has already produced a CycloneDX JSON SBOM at `target/bom.json`.
Create `rio.yaml` beside the project's build files (or adapt your existing manifest):
...
Preview the inputs, then normalize into a fresh output directory:
...
index.json     input/output hashes, transformation counts and gate results
```

`--gate fail` returns exit **1** if required SBOM fields are missing; results are still written for
inspection. Invalid configuration or missing inputs return exit **2** with no new outputs.
A passing gate checks the configured SBOM fields; it does not establish vulnerability status or
inventory completeness. See [exit codes](docs/cli.md#exit-codes) and [output records](docs/output.md).

No SBOM handy? The [offline examples](tools/README.md#agent-integration-examples) include synthetic
inputs and a walkthrough that runs with an installed binary.

## Configure your project

Use **explicit artifacts** for individually named inputs, as in the quick start. Paths are relative
to `rio.yaml`, and each artifact must resolve to exactly one SBOM.
